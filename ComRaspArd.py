...
while True:
	time.sleep(3)
	variable='verde\n'
	mensaje=variable.encode(encoding='UTF-8',errors='strict')
	seri.write(mensaje)
	time.sleep(4)
	variable='rojo\n'
	mensaje=variable.encode(encoding='UTF-8',errors='strict')
	...
	time.sleep(5)	
            ...
            clasificacion=nombreforma+nombrecolor + '\n'
            print(clasificacion)
            mensaje=clasificacion.encode(encoding='UTF-8',errors='strict')
            seri.write(mensaje)
...
def dibujar(mask,color):
    ...
    for c in contornos:
        ...
        if area > 500:
            ...
            nombreforma = 'nada' # lo puse pq sino daba error 
            if len(approx) <= 9:
                nombreforma = 'cubo'
                cv2.putText(frame,"cubo",(x+10,y-20), font, 0.75,color,1,cv2.LINE_AA)
            if len(approx) >= 11:
